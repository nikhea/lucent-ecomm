          ...
          return (
            <Link key={p.id} href={`/products/${p.slug}`} className="group">
              <div className="relative overflow-hidden rounded-xl bg-muted aspect-square border">
                ...
                <span className="absolute top-2 right-2 text-[10px] bg-white text-black px-2 py-1 rounded-full font-medium border">
                  {p.title.includes('Mini') ? 'New' : p.title.includes('Loop') ? '' : p.title.includes('Replacement') ? 'Trending' : ''}
                </span>
              </div>
              <div className="pt-3">
                <div className="text-xs text-muted-foreground">Accessories • {p.title.includes('Mini') ? 'Wireless' : 'Parts'}</div>
